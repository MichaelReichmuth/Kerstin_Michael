...
```{r}
p <- unfaelle_clean_velostrasse_absolute %>%
  group_by(before_after, fahrrd_bet) %>%
  summarise(count = n()) %>%
ggplot(aes(before_after, count, fill = fahrrd_bet)) +
  geom_col(position = "fill") +
  scale_fill_viridis_d(option = "C") +
  ylab("Proportion of Cases") +
  xlab("") +
  ggtitle("Proportional Distribution of Accidents per 'fahrrd_bet'") +
  theme_minimal()

print(p)

ggsave("Plot_9.jpeg", plot = p, width = 12, height = 8, dpi = 600)
```

```{r}
p <- unfaelle_clean_velostrasse_absolute %>%
  group_by(before_after, fussgg_bet) %>%
  summarise(count = n()) %>%
ggplot(aes(before_after, count, fill = fussgg_bet)) +
  geom_col(position = "fill") +
  scale_fill_viridis_d(option = "C") +
  ylab("Proportion of Cases") +
  xlab("") +
  ggtitle("Proportional Distribution of Accidents per 'fussgg_bet'") +
  theme_minimal()

print(p)

ggsave("Plot_10.jpeg", plot = p, width = 12, height = 8, dpi = 600)
```

```{r}
table(unfaelle_clean_velostrasse_absolute$typ, unfaelle_clean_velostrasse_absolute$before_after)
```
...
```{r}
# Version 1
p <- ggplot(unfaelle_prep, aes(x = diff_months_group, y = relative_count)) +

  # Highlight the transition window
  annotate("rect",
           xmin = -1, xmax = 1,
           ymin = -Inf, ymax = Inf,
           fill = "#DCE1FF", alpha = 0.3) +  # light blue tint

  # Raw points in gray
  geom_point(size = 0.7, alpha = 0.3, color = "#343434") +

  # Before transition: dark gray
  geom_smooth(
    data = subset(unfaelle_prep, before_transition == TRUE),
    method = "lm", se = FALSE, size = 2,
    color = "#343434"
  ) +

  # After transition: vibrant blue
  geom_smooth(
    data = subset(unfaelle_prep, after_transition == TRUE),
    method = "lm", se = FALSE, size = 2,
    color = "#0003FF"
  ) +

  # Vertical dashed line at transition (month 0)
  geom_vline(xintercept = 0, linetype = "dashed", color = "#343434", size = 0.7) +
  ...
  scale_x_continuous(
    ...
    labels = function(x) paste0(x / 12, "y")
  ) +
  scale_y_continuous(
  ...
  # Labels
  labs(
    title = "Trends in Accident Rates Before and After Velostreet",
    subtitle = "Accident rates normalized by cyclist volume across Basel",
    x = "Years relative to velostreet implementation",
    y = "Accident rate (%)"
  ) +

  # Clean theme with strong contrast
  theme_minimal(base_size = 15) +
  theme(
    plot.title = element_text(face = "bold", size = 18, hjust = 0.5, color = "#343434"),
    plot.subtitle = element_text(size = 13, hjust = 0.5, color = "#343434"),
    axis.title = element_text(face = "bold", color = "#343434"),
  ...
  annotate("text", x = -30, y = 0.17, label = "Before", color = "#343434", fontface = "bold", size = 4) +
  annotate("text", x = 12, y = 0.09, label = "After", color = "#0003FF", fontface = "bold", size = 4)

print(p)

ggsave("accident_trends_magazine.png", plot = p, width = 16, height = 9, units = "cm", dpi = 400, limitsize = FALSE)
```
...
p <- ggplot(unfaelle_prep, aes(x = diff_months_group, y = relative_count)) +
  ...
  # Labels
  labs(
    title = "Trends in Accident Rates on Velostrassen across Basel",
    subtitle = "Comparison from before and after their implementation",
  ...
  theme(
    plot.title = element_text(face = "bold", size = 18, hjust = 0.5, color = "#343434"),
    plot.subtitle = element_text(size = 13, hjust = 0.5, color = "#343434"),
    axis.title = element_text(face = "bold", size = 13, color = "#343434"),
    axis.text = element_text(color = "#343434"),
    panel.grid.major.x = element_line(color = "gray85", linetype = "dotted"),
    panel.grid.major.y = element_blank(),
    panel.grid.minor = element_blank(),
  ...
  # Inline labels (optional)
  annotate("text", x = -30, y = 0.06, label = "Before", color = "#343434", fontface = "bold", size = 4) +
  annotate("text", x = 12, y = 0.05, label = "After", color = "#343434", fontface = "bold", size = 4)
...
```{r}
ggsave(
  filename = "velostreet_magazine_final.png",
  plot = p,
  width = 18,       # cm (or larger for full slide)
  height = 12,      # cm
  units = "cm",
  dpi = 1200,        # very high resolution
...
```{r}
color_palette <- c(
  "Property Damage Only" = "#343434",
  "Minor Injury" = "#0003FF",
  "Severe Injury" = "#dc3545"
)

plot_data <- unfaelle_clean_velostrasse_absolute %>%
  mutate(schwere_en = case_when(
    schwere == "1 Unfall mit Sachschaden" ~ "Property Damage Only",
    schwere == "2 Unfall mit Leichtverletzten" ~ "Minor Injury",
    schwere == "3 Unfall mit Schwerverletzten" ~ "Severe Injury",
    TRUE ~ NA_character_
  )) %>%
  mutate(schwere_en = factor(
    schwere_en,
    levels = c("Property Damage Only", "Minor Injury", "Severe Injury"))
  ) %>%
  group_by(before_after) %>%
  mutate(total_count = n()) %>%
  ungroup() %>% 
  group_by(before_after, schwere_en) %>%
  summarise(count = n(), total_count = first(total_count), .groups = "drop") %>%
  mutate(
    proportion = count / total_count,
    x_num = as.numeric(factor(before_after)),
    # Reduce the x_pos shift to make the groups closer
    x_pos = case_when(
      before_after == "before_transition" ~ x_num - 0.05,  # Even smaller distance
      before_after == "after_transition" ~ x_num + 0.05   # Even smaller distance
    )
  )

p_severity <- ggplot(plot_data, aes(x = x_num, y = proportion, group = schwere_en, color = schwere_en)) +
  geom_line(size = 1) +
  geom_point(size = 3) +
  geom_text(
    aes(x = x_pos, label = scales::percent(proportion, accuracy = 1)),
    size = 3.2,
    hjust = ifelse(plot_data$before_after == "before_transition", 1, -0.05),  # Adjust further left or right
    vjust = 0.5,  # Keep percentages at the same height as the points
    fontface = "bold",
    show.legend = FALSE
  ) +
  scale_x_continuous(
    breaks = c(1, 2),                   # Keep ticks at "Before" and "After"
    labels = c("Before", "After"),      # Label as Before and After
    limits = c(0.7, 2.3),               # Further reduce space between "Before" and "After"
    expand = c(0, 0)                    # No extra padding on the x-axis
  ) +
  scale_y_continuous(
    labels = scales::percent_format(accuracy = 1),
    limits = c(0, 1),
    expand = expansion(mult = c(0.02, 0.05)),
    breaks = NULL  # Remove y-axis ticks
  ) +
  ...
  theme(
    ...
    legend.text = element_text(size = 9, color = "#343434"),
    legend.direction = "vertical",  # Stack legend vertically
    axis.title = element_blank(),
    axis.text.y = element_blank(),  # Remove y-axis text
    axis.ticks.y = element_blank(),  # Remove y-axis ticks
    axis.text.x = element_text(color = "#343434", size = 10),
    panel.grid = element_blank(),
    plot.margin = margin(5, 10, 5, 5),  # Adjust bottom margin for legend
    legend.spacing.y = unit(0.3, "cm"),  # Space between legend items
    legend.box.margin = margin(0, 0, 10, 0),  # Add some margin to the legend box
...
```{r}
ggsave(
  filename = "severity_final.png",
  plot = p_severity,
  width = 6,       # cm (or larger for full slide)
  height = 12,      # cm
  units = "cm",
...
highlight_groups <- c("Minor Injury", "Severe Injury")

# Define color palette: highlight two, mute others
color_palette <- c(
  "Property Damage Only" = "#343434",  # muted gray
  "Minor Injury" = "#0003FF",          # highlight blue
  "Severe Injury" = "#dc3545",         # highlight red
  "Fatal Injury" = "#343434"                 # muted gray
)

# Define shape palette to distinguish all groups
shape_palette <- c(
  "Property Damage Only" = 15,  # square → stable, no injury
  "Minor Injury" = 17,          # triangle → caution/warning
  "Severe Injury" = 8,          # asterisk/star → critical/impact
  "Fatal Injury" = 4                  # cross → symbolic of fatality
)
...
plot_data <- unfaelle_clean_velostrasse_absolute %>%
  ...
  mutate(
    ...
    x_num = as.numeric(factor(before_after)),
    x_pos = case_when(
      before_after == "before_transition" ~ x_num - 0.05,
      before_after == "after_transition" ~ x_num + 0.05
    )
  )

# Add 0% "Deaths" line
deaths_data <- data.frame(
  before_after = c("before_transition", "after_transition"),
  schwere_en = "Fatal Injury",
  count = 0,
  total_count = plot_data$total_count[match(c("before_transition", "after_transition"), plot_data$before_after)],
  proportion = 0,
  x_num = c(1, 2),
  x_pos = c(0.95, 2.05)
)
...
plot_data <- bind_rows(plot_data, deaths_data) %>%
  mutate(schwere_en = factor(
    schwere_en,
    levels = c("Property Damage Only", "Minor Injury", "Severe Injury", "Fatal Injury"))
  )

# Plot
p_severity <- ggplot(plot_data, aes(x = x_num, y = proportion, group = schwere_en)) +
  geom_line(aes(color = schwere_en), size = 1) +
  geom_point(aes(color = schwere_en, shape = schwere_en), size = 3) +
  geom_text(
    aes(x = x_pos, label = scales::percent(proportion, accuracy = 1), color = schwere_en),
    size = 3.2,
    hjust = ifelse(plot_data$before_after == "before_transition", 1, -0.05),
    vjust = 0.5,
    fontface = "bold",
    show.legend = FALSE
  ) +
  ...
  scale_y_continuous(
    labels = scales::percent_format(accuracy = 1),
    limits = c(0, 1),
    expand = expansion(mult = c(0.07, 0.05)),  # give space for 0%
    breaks = NULL
  ) +
  scale_color_manual(values = color_palette) +
...
```{r}
combined_plot <- p + p_severity + plot_layout(widths = c(3, 1))
...
ggsave(
  filename = "test.png",
  plot = combined_plot,
  width = 20,       # cm (or larger for full slide)
  height = 12,      # cm
  units = "cm",
  dpi = 600,        # very high resolution
  limitsize = FALSE, # allows large plots
...
```{r}
# Step 1: Factorize 'typ'
unfaelle_clean_velostrasse_absolute <- unfaelle_clean_velostrasse_absolute %>%
  mutate(typ = as.factor(typ))

# Step 2: Calculate total counts per period
total_counts <- unfaelle_clean_velostrasse_absolute %>%
  count(before_after, name = "total_count")

# Step 3: Count occurrences of each 'typ' per period
plot_data_typ <- unfaelle_clean_velostrasse_absolute %>%
  count(before_after, typ, name = "count") %>%
  complete(before_after, typ, fill = list(count = 0)) %>%  # fill missing combos with 0
  left_join(total_counts, by = "before_after") %>%         # add total count
  mutate(
    proportion = count / total_count,
    x_num = as.numeric(factor(before_after, levels = c("before_transition", "after_transition"))),
    x_pos = case_when(
      before_after == "before_transition" ~ x_num - 0.05,
      before_after == "after_transition" ~ x_num + 0.05
    )
